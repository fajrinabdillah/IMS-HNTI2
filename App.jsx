...
const translations = {
  id: {
    ...
    // YoY
    <Tooltip content={<ChartTooltip fmt={fmt} />} />: 'Pertumbuhan Tahun ke Tahun', yoy_subtitle: 'Perbandingan SPH dan PO antar tahun',
...
const storeGet = async (k) => { try { const r = localStorage.getItem(k); return r; } catch { return null; } }; const storeSet = async (k, v) => { try { localStorage.setItem(k, v); } catch {} }; const storeDel = async (k) => { try { localStorage.removeItem(k); } catch {} }; (k) => { try { const r = await window.storage.get(k); return r?.value; } catch { return null; } };
...
function getIncentiveStatus(sph) {   if (sph.poStatus !== 'issued') return null;   const isGovOrTender = sph.projectType === 'government' || sph.projectType === 'tender';   const isKSO = sph.projectType === 'kso';   const isKsoSelfInvest = isKSO && sph.ksoMode === 'hnti_self_invest';    if (isKsoSelfInvest) {     const monthsPaid = sph.ksoMonthsPaid || 0;     if (monthsPaid >= 12) return { status: 'paid', label: 'inc_status_paid', color: '#3a6b3a' };     if (monthsPaid > 0) return { status: 'kso_prorata', label: 'inc_status_kso_prorata', color: '#c8a96a', progress: monthsPaid / 12 };     return { status: 'estimated', label: 'inc_status_estimated', color: '#94a3b8' };   }    if (isGovOrTender) {     if (sph.finalPaid) return { status: 'paid', label: 'inc_status_paid', color: '#3a6b3a' };     if (sph.dpPaid || sph.installationStatus === 'installed') return { status: 'ready', label: 'inc_status_ready', color: '#c8a96a' };     return { status: 'estimated', label: 'inc_status_estimated', color: '#94a3b8' };   }    const pct = sph.paymentReceivedPct || (sph.finalPaid ? 100 : sph.dpPaid ? 50 : 0);   if (pct >= 100) return { status: 'paid', label: 'inc_status_paid', color: '#3a6b3a' };   if (pct >= 50) return { status: 'ready', label: 'inc_status_ready', color: '#c8a96a' };   return { status: 'estimated', label: 'inc_status_estimated', color: '#94a3b8' }; }(sph) {
            ...
            <div style={{fontSize: '13px', lineHeight: 1.7}}>
              ...
              <div style={{padding: '10px 12px', background: 'rgba(200,169,106,0.10)', border: '1px dashed #c8a96a', marginTop: '4px', marginBottom: '4px'}}>
                ...
                <div style={{display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap'}}>
                  ...
                  <input type="number" step="0.5" min="0" max="50" value={((selectedDeal.opsPercent !== undefined ? selectedDeal.opsPercent : 0.05) * 100)}  onChange={(e) => updateOpsPercent(selectedDeal.id, (parseFloat(e.target.value) || 0) / 100)}.toFixed(1)} onChange={(e) => updateOpsPercent(selectedDeal.id, (parseFloat(e.target.value) || 0) / 100)} style={{width: '90px', padding: '6px 10px', fontSize: '13px', border: '1px solid #c8a96a', fontWeight: 600}} />
