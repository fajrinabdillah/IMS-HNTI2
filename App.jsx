...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // V48: muat laporan lapangan tim sales Juni–Juli 2026 (dari CSV Excel)
      const V48_JUN_JUL_REPORTS_MARKER = 'ims_hnti:v48_jun_jul_field_reports';
      const v48JunJulReports = await storeGet(V48_JUN_JUL_REPORTS_MARKER);
      if (!v48JunJulReports) {
        const repStored = await storeGet(REPORTS_KEY);
        let existing = [];
        try { existing = repStored ? JSON.parse(repStored) : []; } catch {}
        if (!Array.isArray(existing) || !existing.length) existing = [...SEED_FIELD_REPORTS];
        const byId = new Map(existing.map(r => [r.id, r]));
        for (const r of FIELD_REPORTS_JUN_JUL_2026) byId.set(r.id, r);
        const merged = [...byId.values()].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
        await storeSet(REPORTS_KEY, JSON.stringify(merged));
        await storeSet(V48_JUN_JUL_REPORTS_MARKER, 'true');
      }

      // V49: tambah profil Astrika (non-aktif s/d Agustus) & perbarui referensi karyawan sales
      const V49_TERRITORY_MARKER = 'ims_hnti:v49_territory_astrika_employee';
      const v49Territory = await storeGet(V49_TERRITORY_MARKER);
      if (!v49Territory) {
        const empStored = await storeGet('ims_hnti:emp_v22');
        let emp = {};
